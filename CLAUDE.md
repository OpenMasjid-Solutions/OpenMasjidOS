...
- Human sending behaviour, all of it load-bearing: randomised 6–20s gaps (a fixed interval is itself a fingerprint), a typing indicator scaled to message length then `paused`, presence online-while-working, a per-recipient cooldown, rolling hour/day caps **for groups only**, a warm-up ramp for a freshly linked number, and `contacts/check` before first contact (messaging a number that is not on WhatsApp is a documented ban signal). The policy is pure and unit-tested (`blockedReason`, `warmupFactor`), and `clampLimits` bounds what an admin can set — within a range, so caps can be raised as well as lowered, but never past the ceilings and never below a 3s gap.
  - **INDIVIDUAL messages have NO hourly/daily cap** (removed v0.51.1, maintainer's decision). With the warm-up ramp the old 12/hour became **3/hour** on a freshly linked number — and command replies spend the same allowance — so it blocked ordinary use and even blocked testing the feature, for a pattern (one parent at a time) it was never aimed at. Spacing is the brake: the 6–20s randomised gap plus the 60s per-recipient cooldown. **The trade-off is real and is recorded rather than hidden:** the queue is shared, so an app looping over 200 parents now sends all 200, spaced but unbounded, and a ban is terminal. If a ceiling is needed again it belongs on the shared queue in `capExceeded`, never per-app — a per-app limiter cannot see the number's total traffic. **Group caps SURVIVE** (4/hr, 10/day, 30-min cooldown, ramp applied): one post reaches every member, so overuse costs recipients who did not choose it, which is not the same as an over-eager fee run.
