...
| OPENMASJIDOS-004 | File Explorer's sandbox root is the data dir, so any session reads `stripe.json`, `email.json`, the tunnel token and the TLS key — and rewrites any app's compose | confirmed (6 auditors) | `packages/core/src/files/manager.ts:36` | **fixed** in `fix/files-protect-platform-state` (before/after demonstrated against the published image; backup+restore round trip re-verified — see `REMEDIATION.md`) |
